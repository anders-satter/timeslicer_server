package timeslicer.models.input

import scala.collection.mutable.ListBuffer
import timeslicer.models.util.DateTime
import timeslicer.models.util.FileUtil
import timeslicer.model.util.settings.Settings
import timeslicer.models.util.ItemUtil
import timeslicer.models.log.Item
import timeslicer.models.reporting.Activity
...
//  }
  ...
  def getActivitiesForProject(projectName:String):ListBuffer[String] = {
    
    val activityList = currentProjectBuffer.filter(_.name == projectName).
      map(i => i.activities.sortBy ( x => x.name.toLowerCase()).map(_.name) )
